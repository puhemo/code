...
published: true
excerpt: |
     After seeing a Docker presentation recently I decided to finally figure out how to get Docker working correctly on Windows.  These instructions use the Docker Toolkit and Virtualbox.
---
...
1. Arrow down to the end of the file and press i to enter insert mode which allows you to add text in the file.
          ...
          INTEL_PROXY='http://[Proxy Server]:[Proxy Port]'
          export {http,https,ftp,socks,all,npm_config,npm_config_https}_{proxy,PROXY}=$INTEL_PROXY
          export {HTTP,HTTPS,FTP,SOCKS,ALL}_{proxy,PROXY}=$INTEL_PROXY
          export {no,NO}_{proxy,PROXY}=intel.com,.intel.com,10.0.0.0/8,192.168.0.0/16,localhost,127.0.0.0/8,134.134.0.0/16,192.168.99.100
...
1. Arrow down to the end of the file and press i to enter insert mode which allows you to add text in the file.
          ...
          export NO_PROXY=intel.com,.intel.com,10.0.0.0/8,192.168.0.0/16,localhost,127.0.0
...
Unfortunately even with auto-mount Docker will only mount the c/Users folder in the docker-machine.  If you want the folders to auto-mount you will need to modify the boot2docker profile.

1. Login to the docker machine and edit the boot2docker profile
...
Once the reboot is done, the shared folders will be auto mounted.  To verify that it worked, ssh into the docker machine and you should be able to do an ls /c/projects or /c/personal and see all of the directories from the your local machine from within the docker-machine default.
